...
local keys = { Down = C.GBA_KEY.DOWN, A = C.GBA_KEY.A, B = C.GBA_KEY.B, L = C.GBA_KEY.L, R = C.GBA_KEY.R, Up = C.GBA_KEY.UP, Right = C.GBA_KEY.RIGHT, Left = C.GBA_KEY.LEFT, Start = C.GBA_KEY.START }
local key = assert(keys[key_name], "input key must be Down, Up, Left, Right, A, B, L, R, or Start")
local frame = 0
