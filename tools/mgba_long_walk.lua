...
local function log(line)
	local f = io.open(OUT_DIR .. "/mgba-longwalk.log", frame == 0 and "w" or "a")
	if f then
		f:write(line .. "\n")
		f:close()
	end
...
local function screenshot(tag)
	local name = string.format("%s/longwalk-%04d.png", OUT_DIR, frame)
	emu:screenshot(name)
...
callbacks:add("frame", function()
	...
	keys = keys | pulse(KEY_DOWN, 1080, 6)
	keys = keys | pulse(KEY_A, 1140, 8)
	keys = keys | pulse(KEY_START, 1320, 8)
	keys = keys | pulse(KEY_DOWN, 1440, 6)
	keys = keys | pulse(KEY_A, 1500, 8)
	...
	if frame == 1680 then
		screenshot("final")
		local f = io.open(OUT_DIR .. "/mgba-longwalk.done", "w")
		if f then
			f:write("done\n")
			f:close()
		end
