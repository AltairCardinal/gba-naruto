...
从恢复态选择“移动→对战”后先进入 Naruto/木叶丸长对白；约 21 次确认才到队伍·装备
页面，期间没有战斗、经验和 A880 变化。因此“Continue 后立即下一战”被否决。后续
导航必须检测 dialogue 并逐次确认，到队伍界面立即停止，随后确认/退出队伍并再次移动，
再寻找真正战斗。升级 checkpoint 判据保持：A880=3、Naruto level 2、分配前
`+BA=1`，并命中 `0x0808E16E→0x08093698`。

进一步确认该“队伍·装备”页属于下一任务准备流程；按 B 可回到包含“队伍·装备 / 查看
地图 / 开始任务 / 保存”的任务准备菜单。选择“开始任务”仍会回到队伍配置，说明存在
队伍确认/配置前置条件。跨 ss9 normalized replay 时选中索引不能仅靠截图推断，下一步
必须在同一会话逐键读取状态反馈，闭合队伍条件后才能进入新战场。
