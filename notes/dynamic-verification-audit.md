...
|---:|---|---|---|
| 1 | audio (`0x465B70`) | runtime | `0x0809AAC0` indexes the sound-ID master table and initializes m4a tracks; a live wrapper probe captured ID 118 resolving to descriptor `0x53D06C`. All 217 tracks structurally decode; 80 one-loop MIDIs execute 17,202 original events plus 21,328 pitch-state and 449 mix-state events. Instrument coverage resolves 16,180/16,180 notes: 16,169 DirectSound notes reach all 79 waves (5,340 through drum tables), and 11 are explicit PSG/noise. Runtime NoteRequest now reaches terminal tone and 10+4 channel allocation; real DirectSound/CGB vectors, drum-root pitch, active-note dirty propagation, natural-stop unlink and a first 264-frame PCM hash are locked. TIE lifecycle is 25 EOT / 19 FINE / 46 loop-open. Player-level full-song scheduling, CGB+Direct combined PCM, emulator-buffer differential and audible cue names remain open. |
