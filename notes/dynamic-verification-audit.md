...
|---:|---|---|---|
| 1 | audio (`0x465B70`) | runtime | `0x0809AAC0` indexes the sound-ID master table and initializes m4a tracks; a live wrapper probe captured ID 118 resolving to descriptor `0x53D06C`. All 217 tracks structurally decode; 80 one-loop MIDIs execute 17,202 original events plus 21,328 pitch-state and 449 mix-state events. Instrument coverage resolves 16,180/16,180 notes: 16,169 DirectSound notes reach all 79 waves (5,340 through drum tables), and 11 are explicit PSG/noise. ROM-exact pitch covers all note-ons and 30,937 active-note updates; two-stage volume/pan covers all DirectSound notes and 39 active-note VOL updates. ADSR/release/pseudo-echo/master gain covers all DirectSound notes across four valid tuples. TIE lifecycle is 25 EOT / 19 FINE / 46 loop-open. PSG, full mixer-buffer PCM output and audible cue names remain open. |
