...
- [ ] 3.18 让独立审查者检查 wrapper 调用约定、ROM diff、截图/消息因果和证据等级；处理意见后，将批次 A 的完整闭环作为独立提交推送远端。
...
- [ ] 4.9 经独立视觉证据与 ROM diff 审查后，将批次 B 的完整闭环作为独立提交推送远端。
...
- [ ] 5.9 经独立回调安全、资源兼容和证据等级审查后，将批次 C 的完整闭环作为独立提交推送远端。
...
- [ ] 6.6 经独立音频因果与代码完整性审查并通过相关测试后，将批次 D 的完整结论作为独立提交推送远端。
...
- [ ] 7.6 检查 `git status`，确保不混入 `levels`、P2 audio cue、legacy web/ROM mirror 或其他用户改动；提交最终审计收口并推送远端。
