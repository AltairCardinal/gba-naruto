...
- [x] 4.1 从 hook 前快照运行三 site observer，证明 `0x08073940/0x08073946` selector 自然命中，并以 legacy PCU 或 canonical PCA 将当前 slot 绑定到玩家单位
...
- [x] 4.4 固化首个玩家控制检查点与紧凑 JSON 证据

## 5. 闭合自然行动、胜利与 postbattle

- [ ] 5.1 从玩家控制快照辨识实际 acting unit 并完成第一回合自然行动，捕获 MOVEDONE 与坐标/回合状态变化
