...
def _rgb8_png(red: int, green: int, blue: int) -> bytes:
...
class FrameDirectoryTests(unittest.TestCase):
    ...
    def test_analyzer_reuses_strict_png_fingerprint_for_all_600_frames(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fingerprint = {"rgb_pixels_sha256": "a" * 64}
...
class CliTests(unittest.TestCase):
            ...
            json.dumps(
                {
                    ...
                    "capture_frame": 600,
                    "pre_scripts": [
                        {"path": str(SAMPLER.resolve()), "sha256": _sha256(SAMPLER)}
                    ],
                }
            ),
            encoding="utf-8",
    ...
    def test_cli_validates_provenance_and_writes_complete_cycle_json(self):
        ...
        expected_rgb = hashlib.sha256(bytes((1, 2, 3))).hexdigest()
        ...
        self.assertEqual(report["sampler"]["sha256"], _sha256(SAMPLER))
